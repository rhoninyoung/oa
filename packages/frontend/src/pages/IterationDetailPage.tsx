...
export default function IterationDetailPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/iterations/${iterationId}`, { headers: { 'x-user-id': currentUser.id } })
      ...
      <div className="space-y-2">
        {schedules.map((s) => (
          ...
          >
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded ${s.status === 'APPROVED' ? 'bg-green-100 text-green-700' : s.status === 'REVIEWING' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}
            >
              {s.status}
            </span>
          </div>
