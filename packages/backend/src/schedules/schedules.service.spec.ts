...
describe('SchedulesService', () => {
  ...
  beforeEach(() => {
    ...
    svc = new SchedulesService(mockPrisma as any, mockOutbox);
    jest.clearAllMocks();
    (canTransition as jest.Mock).mockReturnValue({ ok: true });
  });
  ...
  it('UT-SCH-03: submit checks canTransition(PENDING → REVIEWING, GROUP_LEADER)', async () => {
    ...
    expect(canTransition).toHaveBeenCalledWith('PENDING', 'REVIEWING', 'GROUP_LEADER', {
      tasksNonEmpty: true,
    });
  });
  ...
  it('UT-SCH-04: submit throws BadRequestException when canTransition rejects', async () => {
    (canTransition as jest.Mock).mockReturnValue({ ok: false, code: 'CONTENT_EMPTY' });
    mockPrisma.groupSchedule.findUnique.mockResolvedValue({
      id: 'sched-1',
      status: 'PENDING',
      tasks: [],
  ...
  it('UT-SCH-10: reschedule transitions to REJECTED and emits RESCHEDULE event', async () => {
    ...
    expect(canTransition).toHaveBeenCalledWith('APPROVED', 'REJECTED', 'PROJECT_MANAGER', {});
