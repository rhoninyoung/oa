...
export class MasterService {
  ...
  async getMasterView(iterationId: string, userId: string) {
  ...
  async addMasterRow(iterationId: string, ownerId: string, userId: string) {
    ...
    if (!user || user.role !== 'PROJECT_MANAGER') throw new ForbiddenException({ code: 'ACTOR_NOT_PM' });
    ...
    const maxIdx = await this.prisma.task.aggregate({ where: { scheduleId: schedule.id }, _max: { orderIndex: true } });
    const task = await this.prisma.task.create({
      data: { scheduleId: schedule.id, orderIndex: (maxIdx._max.orderIndex ?? 0) + 1, name: '', ownerId, source: 'MASTER' },
    });
