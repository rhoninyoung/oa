...
describe('canTransition', () => {
    ...
    it('DT-SM-01: PENDING → REVIEWING by GL, non-empty tasks → allowed', () => {
        const result = (0, stateMachine_js_1.canTransition)('PENDING', 'REVIEWING', 'GROUP_LEADER', {
            tasksNonEmpty: true,
        });
    ...
    it('DT-SM-03: REJECTED → REVIEWING by GL → allowed (resubmit)', () => {
        const result = (0, stateMachine_js_1.canTransition)('REJECTED', 'REVIEWING', 'GROUP_LEADER', {
            tasksNonEmpty: true,
        });
        expect(result).toEqual({ ok: true });
    });
    ...
    it('DT-SM-04: APPROVED → REVIEWING by GL → allowed (re-submit)', () => {
        const result = (0, stateMachine_js_1.canTransition)('APPROVED', 'REVIEWING', 'GROUP_LEADER', {
            tasksNonEmpty: true,
        });
        expect(result).toEqual({ ok: true });
    });
    ...
    it('DT-SM-05: REVIEWING → PENDING by GL (withdraw) → allowed', () => {
        const result = (0, stateMachine_js_1.canTransition)('REVIEWING', 'PENDING', 'GROUP_LEADER', {});
        expect(result).toEqual({ ok: true });
    });
    // ── DT-SM-06: REVIEWING → PENDING by PM → ACTOR_NOT_OWNER ─────────────────
    it('DT-SM-06: REVIEWING → PENDING by PM → ACTOR_NOT_OWNER', () => {
        const result = (0, stateMachine_js_1.canTransition)('REVIEWING', 'PENDING', 'PROJECT_MANAGER', {});
    ...
    it('DT-SM-07: REVIEWING → APPROVED by PM → allowed', () => {
        const result = (0, stateMachine_js_1.canTransition)('REVIEWING', 'APPROVED', 'PROJECT_MANAGER', {});
        expect(result).toEqual({ ok: true });
    });
    // ── DT-SM-08: REVIEWING → APPROVED by GL → ACTOR_NOT_PM ───────────────────
    it('DT-SM-08: REVIEWING → APPROVED by GL → ACTOR_NOT_PM', () => {
        const result = (0, stateMachine_js_1.canTransition)('REVIEWING', 'APPROVED', 'GROUP_LEADER', {});
    ...
    it('DT-SM-09: REVIEWING → REJECTED by PM, reason in [1,200] → allowed', () => {
        const result = (0, stateMachine_js_1.canTransition)('REVIEWING', 'REJECTED', 'PROJECT_MANAGER', {
            rejectReason: '需求不合理',
        });
        expect(result).toEqual({ ok: true });
    });
    ...
    it('DT-SM-11: APPROVED → REJECTED by PM (reschedule) → allowed', () => {
        const result = (0, stateMachine_js_1.canTransition)('APPROVED', 'REJECTED', 'PROJECT_MANAGER', {});
        expect(result).toEqual({ ok: true });
    });
    ...
    it('DT-SM-12: PENDING → APPROVED (skip REVIEWING) → INVALID_TRANSITION', () => {
        const result = (0, stateMachine_js_1.canTransition)('PENDING', 'APPROVED', 'PROJECT_MANAGER', {});
