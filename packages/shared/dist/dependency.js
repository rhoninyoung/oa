...
function setDependency(taskId, depId, existingTasks) {
    const task = existingTasks.find(t => t.id === taskId);
    if (!task)
        return { ok: false, code: 'CYCLE', cyclePath: [] };
    if (depId !== null) {
        // 1-to-1 violation
        ...
        // Cycle check
        const tempTasks = existingTasks.map(t => t.id === taskId ? { ...t, dependencyTaskId: depId } : t);
    ...
    const updatedTasks = existingTasks.map(t => t.id === taskId ? { ...t, dependencyTaskId: depId } : t);
